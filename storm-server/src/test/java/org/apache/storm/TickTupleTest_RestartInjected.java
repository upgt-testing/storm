...
public class TickTupleTest_RestartInjected {
    private final static Logger LOG = LoggerFactory.getLogger(TickTupleTest_RestartInjected.class);
    ...
    public void testTickTupleWorksWithSystemBolt() throws Exception {
        try (ILocalCluster cluster = new LocalCluster.Builder()
            ...
            try (ILocalTopology ignored = cluster.submitTopology("test", topoConf, builder.createTopology())) {
                ...
                RestartFramework.at("after_first_tuple_ack")
                    .on(cluster)
                    .restart("worker")
                    .withIndex(0)
                    .withMode(RestartMode.GRACEFUL)
                ...
                try {
                    ...
                    RestartFramework.at("after_first_time_advance")
                        .on(cluster)
                        .restart("worker")
                        .withIndex(0)
                        .withMode(RestartMode.GRACEFUL)
                    ...
                    waitForTicks(1);
                    cluster.advanceClusterTime(TICK_INTERVAL_SECS);
                    RestartFramework.at("after_second_time_advance")
                        .on(cluster)
                        .restart("supervisor")
                        .withIndex(0)
                        .withMode(RestartMode.GRACEFUL)
